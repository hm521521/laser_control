    ...
    foreach(QNetworkInterface iface, QNetworkInterface::allInterfaces())
    {
        foreach (QNetworkAddressEntry entry, iface.addressEntries())
        {
            ...
            if (addr.protocol() != QAbstractSocket::IPv6Protocol)
            {
                ...
                {
                    qDebug()<<tmpIO.iface<<" "<<tmpIO.address<<" "<<tmpIO.controller;
...
bool ArtNetPlugin::openInput(quint32 input, quint32 universe)
{
    ...
    if (m_IOmapping[input].controller == NULL)
    {
        ...
        connect(controller, SIGNAL(valueChanged(quint32,quint32,quint32,uchar)),
                this, SIGNAL(valueChanged(quint32,quint32,quint32,uchar)));
        connect(controller, SIGNAL(universeDataChanged(quint32,quint32,QByteArray)),
                this, SIGNAL(universeDataChanged(quint32,quint32,QByteArray)));
