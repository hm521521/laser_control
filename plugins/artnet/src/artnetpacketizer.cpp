...
void ArtNetPacketizer::setupArtNetPollReply(QByteArray &data, QHostAddress ipAddr, QString MACaddr)
{
    ...
    std::string shortName = "yelWinchSim";
    data.append(shortName);   // Short Name
    for (i = 0; i < 18-shortName.size(); i++)
        data.append((char)0x00); // stuffing
    std::string longName = "Yel Winch Simulation - ArtNet interface";
    data.append(longName); // Long Name
    for (i = 0; i < 64-longName.size(); i++) // the length of the long name
